...
- **Semantic search (on-device, optional):** a brain toggle beside the search box ranks memos by meaning instead of substring — "水果" finds the memo that only mentions 苹果和香蕉. Everything runs in the browser: a small bge-small-zh-v1.5 embedding model (~24 MB, downloaded once from this repository's release assets with a pinned Hugging Face fallback, verified against SHA-256 checksums baked into the app) runs on same-origin WebAssembly, and no memo text ever leaves the device. Model weights persist across logouts as public artifacts; the vector index is derived from memo content, so it is sealed with the same server-held key as the snapshot and deleted on logout. Enable it from Settings → Semantic search (download, per-file export, and content-hash-matched import for offline devices live there). See `docs/embedding-model-hosting.md` for the hosting design.
